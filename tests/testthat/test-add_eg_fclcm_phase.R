testthat::test_that("test error", {
  test_df <- data.frame(
    lcsi_cat = c("None", "Stress", "Crisis", "Emergency", "None", "Crisis", "Stress"),
    fc_phase = c("Phase 1 FC", "Phase 2 FC", "Phase 3 FC", "Phase 3 FC", "Phase 4 FC", "Phase 4 FC", "Phase 5 FC")
  )

  testthat::expect_error(add_eg_fclcm_phase(test_df, fc_phase_var = "fcs"), "fcs column is not found in the dataset")
  testthat::expect_error(add_eg_fclcm_phase(test_df, lcs_cat_var = "lsci"), "lsci column is not found in the dataset")
  ...
  testthat::expect_error(add_eg_fclcm_phase(test_df, fc_phase_1 = NULL))


  ## CHECK WARNING::Potential missing entry in the dataset
  test_df <- data.frame(
    lcsi_cat = c("Stress", "Crisis", "Emergency", "None", "Crisis", "Stress"),
    fc_phase = c("Phase 2 FC", "Phase 3 FC", "Phase 3 FC", "Phase 4 FC", "Phase 4 FC", "Phase 5 FC")
  )
  testthat::expect_warning(add_eg_fclcm_phase(test_df))

  testthat::expect_error(add_eg_fclcm_phase(test_df, lcs_cat_none = "phase__1"))

  ## additional entry

  test_df <- data.frame(
    lcsi_cat = c("Nonee", "Stress", "Crisis", "Emergency", "None", "Crisis", "Stress"),
    fc_phase = c("Phase 1 FC", "Phase 2 FC", "Phase 3 FC", "Phase 3 FC", "Phase 4 FC", "Phase 4 FC", "Phase 5 FC")
  )

  testthat::expect_error(add_eg_fclcm_phase(test_df))
...
testthat::test_that("test expect equal", {
  ...
  expected_output <- test_df %>%
    dplyr::mutate(fclcm_phase = dplyr::case_when(
      lcsi_cat == "None" & fc_phase == "Phase 1 FC" ~ "Phase 1 FCLC",
  ...
  actual_df <- test_df |> add_eg_fclcm_phase()
  ...
  renamed_actual_df <- renamed_test_df |> add_eg_fclcm_phase(
  ...
  renamed_actual_df <- lower_values_test_df |> add_eg_fclcm_phase(
...
testthat::test_that("NA  check", {
  test_df <- data.frame(
    lcsi_cat = c("None", "Stress", "Crisis", "Emergency", "None", "Crisis", "Stress", NA_character_),
  ...
  testthat::expect_warning(add_eg_fclcm_phase(test_df))
  actual_df <- test_df |> add_eg_fclcm_phase() |> suppressWarnings()
  testthat::expect_equal(actual_df, expected_output)
