...
testthat::test_that("Check add_fcs",{



 uuid_issue <- c("e7da37c0-dd23-4d38-8cac-2e8e8a243b57", "f6b056c8-bda7-43d2-a723-582dc8001265",
  ...
  df_issue_removed <- food_consumption_df %>% filter(!X_uuid %in% uuid_issue) ### getting rid of potential issue


  res<-evaluate_promise(add_fcs(dataset = df_issue_removed,var_name = "fcs_test",
  ...
  res_unclean<-evaluate_promise(add_fcs(dataset = food_consumption_df,var_name = "fcs_test",
                                 cereals = "cereals_grains_roots_tubers",
                                 pulses = "beans_legumes_pulses_nuts",
                                 dairy = "milk_dairy_products",
                                 meat ="meat_fish_eggs",
...
testthat::test_that("add_hhs",{
  ...
  add_result <- evaluate_promise(
    add_hhs(dataset = food_consumption_df,new_colname = "hhs",
  ...
  expect_error(add_hhs(dataset = food_consumption_df,new_colname = "hhs",
