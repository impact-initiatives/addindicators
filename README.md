...
``` r
df_with_hhs <- df_with_fcs %>% add_eg_hhs(
...
``` r
df_with_lcsi <- df_with_hhs %>% add_eg_lcsi(
...
``` r
df_with_rcsi <- df_with_lcsi %>% add_eg_rcsi(
...
``` r
review_df <- addindicators_MSNA_template_data %>% add_eg_fcs(
  cutoffs = "normal",
  fsl_fcs_cereal = "fs_fcs_cereals_grains_roots_tubers",
  fsl_fcs_legumes = "fs_fcs_beans_nuts",
  fsl_fcs_veg = "fs_fcs_vegetables_leaves",
