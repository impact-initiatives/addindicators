...
#' )
#' add_fcs(df1,
#'   cutoffs = "normal 21.5-35", fcs_cereal = "cereal", fcs_legumes = "pulses", fcs_veg = "vegetables",
...
add_fcs <- function(df,
  ...
  df <- df %>%
    dplyr::mutate(
      fcs_weight_cereal1 = ifelse(is.na(df[[fcs_cereal]]), NA, df[[fcs_cereal]] * 2),
